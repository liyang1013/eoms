...
export default {
  ...
  methods: {
    remoteMethod(str){
      if(  this.pjb01 && str){
        this.$http.post('/api/pjb/searchPjbList',{centre: this.centre,pja: this.pjb01,pjb: str,acti: 'Y'}).then(res => this.pjb = res.data.result)
      }
    }
  },
  created() {
    if( this.pjb01 && this.pjb02 ){
      this.$http.post('/api/pjb/searchPjbByKey',{centre: this.centre,pja: this.pjb01,pjb: this.pjb02,acti: 'Y'}).then(res => this.pjb.push(res.data.result))
    }
  }
}
</script>